...
function init(mode, lprfnc)
	...
	if type(rtos.get_build_time) == "function" then
		log.info("core build time", rtos.get_build_time());
	end;
	if mode == 1 then
...
function timerStart(fnc, ms, ...)
	...
	if rtos.timer_start(msgId, ms) ~= 1 then
		log.debug("rtos.timer_start error");
		return;
	end;
	if argcnt ~= 0 then
	...
	while true do
		...
		if msg == rtos.MSG_TIMER and timerPool[param] then
			if param <= TASK_TIMER_ID_MAX then
				...
				if loop[param] then
					rtos.timer_start(param, loop[param]);
				end;
			end;
